...
def segment_data_by(df, segmentation_column:str) -> list:
    log.debug(f"splittering historical data for different {segmentation_column} column values");
...
def segments_route(dfs:list) -> list:
    log.debug(f"Redefining route in segments")
    return [df for f, t, segment, df in dfs];


def group_segments(dfs:list) -> dict :
    log.debug(f"Group statisics by segments")
    gdfs = {};
...
def execute(df, segmentation_column:str, ignore_segments:list=None):
    df_source = df[~df[segmentation_column].isin(ignore_segments)] if ignore_segments else df;
